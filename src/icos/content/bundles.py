...
def iter_pack_json_files(pack_root: Path) -> List[SourceFile]:
    ...
    manifest.json is ignored; only endpoint folders are compiled.
    """
...
def bundle_pack(pack_root: Path, out_db: Path) -> str:
    """
    Compile a content pack folder into a bundle SQLite DB.
    Returns the computed content hash.
    """
...
def default_bundle_name(pack_root: Path) -> str:
    """
    Determine the bundle DB file name for a pack based on its manifest.
    """
    manifest = read_manifest(pack_root)
